...
if(isset($_POST['soldStatus'])){
	...
	$result = $mysqli->query($sql); 
	if($result){
		$sql2 = "UPDATE property_reservations SET reservation_status = 'SOLD' WHERE reservation_id = '".$_POST['reservationId']."'";
...
if(isset($_POST['declined'])){
	...
	$result = $mysqli->query($sql); 
	if($result){
		$sql2 = "UPDATE property_reservations SET reservation_status = 'DECLINED' WHERE reservation_id = '".$_POST['reservationId']."'";
		$result = $mysqli->query($sql2);
		if($result){
			$response = json_encode(array('result'=>true));
		}
